<?php

    return [
        [
            'city'      => 'Firenze',
            'address'   => 'Piazza dei Giuochi, 1',
            'name'      => 'Dimora Le Torri',
            'zipcode'   => "50122",
            'description'   => "La Dimora Le Torri è una struttura situata a Firenze, una delle città più affascinanti d'Italia. La dimora è un esempio di architettura storica tipica della regione toscana e offre un'esperienza autentica e lussuosa ai suoi ospiti. La posizione della Dimora Le Torri è ideale per esplorare Firenze e le sue attrazioni principali, come la Galleria degli Uffizi, il Duomo di Santa Maria del Fiore e il Ponte Vecchio, che sono facilmente raggiungibili dalla struttura."
        ],
        [
            'city'      => 'Firenze',
            'address'   => 'Viale Francesco Redi, 63',
            'name'      => 'San Jacopino - Flo Apartments',
            'zipcode'   => "50144",
            'description'   => "San Jacopino Flo Apartments è una struttura situata a Firenze, una delle città più belle e culturalmente ricche d'Italia. Questi appartamenti offrono una soluzione ideale per coloro che desiderano vivere un'esperienza autentica nella città toscana. Gli appartamenti sono arredati in modo accogliente e funzionale, fornendo tutto il necessario per un soggiorno confortevole. Gli interni sono caratterizzati da un design semplice ma elegante, che crea un'atmosfera piacevole e rilassante per gli ospiti."
        ],
        [
            'city'      => 'Firenze',
            'address'   => 'Via Palazzo dei Diavoli, 18',
            'name'      => 'Florence Comfort B B',
            'zipcode'   => "50142",
            'description'   => "Il Florence Comfort B&B è una deliziosa struttura situata nel cuore di Firenze, la città dell'arte e della cultura in Italia. Questo Bed and Breakfast offre un'accoglienza calorosa e un ambiente confortevole per i suoi ospiti. Il personale del Florence Comfort B&B è cordiale e disponibile, pronto ad assistere gli ospiti con informazioni e suggerimenti per esplorare al meglio Firenze. L'atmosfera familiare del B&B contribuisce a creare un'esperienza piacevole e rilassante, ideale per chiunque voglia vivere la città in modo autentico e accogliente."
        ],
        [
            'city'      => 'Firenze',
            'address'   => 'Via dei Banchi, 6',
            'name'      => 'Residenza Mondragone',
            'zipcode'   => "50123",
            'description'   => "La Residenza Mondragone è una splendida struttura situata a Firenze, immersa nel suggestivo paesaggio della campagna toscana. Questa residenza offre un rifugio tranquillo e raffinato per chi desidera vivere un'esperienza di lusso e relax a breve distanza dal centro storico della città. La Residenza Mondragone offre comfort e servizi di alto livello per garantire un soggiorno indimenticabile. Le camere sono spaziose, dotate di arredi eleganti e fornite di servizi moderni, offrendo agli ospiti un ambiente rilassante e di lusso."
        ],
        [
            'city'      => 'Roma',
            'address'   => 'Via del Corso, 292',
            'name'      => 'Sonder Piazza Venezia',
            'zipcode'   => "00186",
            'description'   => "Situato nel centro di Roma, a 100 metri da Piazza Venezia, a 200 metri da Palazzo Venezia e a 700 metri da Largo di Torre Argentina, il Sonder Piazza Venezia propone sistemazioni climatizzate. Nelle vicinanze del Sonder Piazza Venezia potrete visitare alcuni rinomati luoghi d'interesse, tra cui il Pantheon, il Tempio Maggiore e il Quirinale. L’Aeroporto di Roma Ciampino, il più vicino, dista 15 km dall’alloggio."
        ],
        [
            'city'      => 'Roma',
            'address'   => 'Via Tuscolana, 4',
            'name'      => 'Re di Roma - Charme Apartment',
            'zipcode'   => "00173",
            'description'   => "Situato a Roma, a 500 metri dalla Stazione Metro San Giovanni e a meno di 1 km dalla Stazione Metro Ponte Lungo, si trova a 1,2 km da Porta Maggiore e a 1,8 km dalla Stazione Metro Vittorio Emanuele."
        ],
        [
            'city'      => 'Roma',
            'address'   => 'Via del Governo Vecchio, 118',
            'name'      => 'Passepartout',
            'zipcode'   => "00186",
            'description'   => "Il Passepartout vi attende a Roma, nel quartiere Navona. Tra i luoghi di interesse nelle vicinanze scoprirete Piazza Navona, Campo de' Fiori e Sant'Agostino. La proprietà si trova nelle vicinanze di famosi luoghi di interesse, come Largo di Torre Argentina, il Pantheon e Castel Sant'Angelo. L'Aeroporto di Roma-Ciampino, lo scalo più vicino, dista 15 km dal Passepartout."
        ],
        [
            'city'      => 'Roma',
            'address'   => 'Via degli Avignonesi, 20',
            'name'      => 'Daphne Luxury Suites',
            'zipcode'   => "00187",
            'description'   => "Situato a Roma, a 170 m da Piazza Barberini e a 600 m da Piazza di Spagna, il Daphne Luxury Suites ha tra i luoghi di interesse nelle vicinanze includono il colle Quirinal, Via Condotti e Fontana di Trevi. 16 km dall'Aeroporto di Roma-Ciampino, lo scalo più vicino."
        ],
        [
            'city'      => 'Milano',
            'address'   => 'Corso Italia, 11 A',
            'name'      => 'Sonder Missori',
            'zipcode'   => "20122",
            'description'   => "Situato nel centro di Milano, il Sonder Missori offre camere a 4 stelle. Costruito nel 2022, questo hotel a 4 stelle dista 800 metri dal Museo Del Novecento e a 1,4 km dalla Darsena. Soggiornerete a 700 metri dal Palazzo Reale e dal centro."
        ],
        [
            'city'      => 'Milano',
            'address'   => 'Via Feltre, 71',
            'name'      => 'Green Guest House',
            'zipcode'   => "20132",
            'description'   => "Situata a Milano, a 1,5 km dalla Stazione Metro Lambrate, la Green Guest House offre un giardino, camere non fumatori, la connessione WiFi gratuita in tutta la struttura e una terrazza. La struttura si trova a circa 4 km dalla Stazione Metro Centrale, a 4,6 km dalla GAM e a 4,9 km da Villa Necchi Campiglio. Siete invitati ad ammirare la vista sul giardino."
        ],
        [
            'city'      => 'Milano',
            'address'   => 'Via Pergolesi, 31',
            'name'      => 'Milan Apartment Rental',
            'zipcode'   => "20124",
            'description'   => "Ubicato nei pressi della Stazione Centrale di Milano, il Milan Apartment Rental propone moderni appartamenti. Il Milan Apartment Rental si trova a 15 minuti in treno dal centro espositivo Rho Fiera Milano e a 4 fermate con la linea gialla della metropolitana dal Duomo di Milano. "
        ],
        [
            'city'      => 'Milano',
            'address'   => 'Via Bernardino Zenale, 9',
            'name'      => 'Magenta - Zenale Charme Loft',
            'zipcode'   => "20123",
            'description'   => "Located in the centre of Milan, 200 metres from Santa Maria delle Grazie and 200 metres from The Last Supper. This apartment is 1.8 km from La Scala and 1.7 km from Galleria Vittorio Emanuele."
        ],
        [
            'city'      => 'Venezia',
            'address'   => 'Calle Dei Boteri San Polo, 1699',
            'name'      => 'Ai Boteri',
            'zipcode'   => "30125",
            'description'   => "Dotato di camere comfort, Ai Boteri si trova a Venezia, a 300 metri dal Ponte di Rialto e in una zona ricca di bar e ristoranti."
        ],
        [
            'city'      => 'Venezia',
            'address'   => 'Calle Larga Giacinto Gallina, 5400',
            'name'      => 'Ca Cappellis',
            'zipcode'   => "30121",
            'description'   => "Il Ca' Cappellis dista 200 metri dalla Basilica dei Santi Giovanni e Paolo, 10 minuti a piedi da Piazza San Marco e 5 minuti dalla fermata del vaporetto Fondamente Nove, che vanta collegamenti con Murano e l'Aeroporto Marco Polo di Venezia."
        ],
        [
            'city'      => 'Venezia',
            'address'   => 'Cannaregio, 2893',
            'name'      => 'Locanda del Ghetto',
            'zipcode'   => "30121",
            'description'   => "Situata in un edificio del 15° secolo, la Locanda del Ghetto offre camere con pavimenti in parquet nel cuore del Ghetto Veneziano. La struttura sorge proprio sotto la Sinagoga Schola Italiana e serve una ricca colazione, a base principalmente di cibo kosher, La colazione include croissant, cereali, pane, bevande calde e succo d'arancia."
        ],
        [
            'city'      => 'Venezia',
            'address'   => 'Campo S. Maria Formosa, 5240',
            'name'      => 'Hotel Scandinavia - Relais',
            'zipcode'   => "30122",
            'description'   => "L'Hotel Scandinavia offre camere confortevoli e ben arredate, con un mix di arredi classici e moderni per creare un'atmosfera accogliente e rilassante. Le camere possono vantare viste panoramiche sulla città, offrendo un'esperienza unica."
        ],
        [
            'city'      => 'Bologna',
            'address'   => 'Via Claudio Treves, 13',
            'name'      => 'A Casa di Giancarla',
            'zipcode'   => "40135",
            'description'   => "LA CASA DI GIANCARLA dista 4,3 km dal Mambo e 4,4 km dal Quadrilatero di Bologna. L'Aeroporto più vicino è quello di Bologna-Guglielmo Marconi, a 4 km."
        ],
        [
            'city'      => 'Bologna',
            'address'   => 'Via Marsala, 39',
            'name'      => 'Lovely Flat, ground floor center of Bologna',
            'zipcode'   => "40126",
            'description'   => "
            Situato nel centro di Bologna, a 300 m da La Macchina del Tempo e a 700 m dalla Chiesa di Santo Stefano, il Lovely Flat, ground Floor Center of Bologna!:) offre la connessione WiFi gratuita. Questo appartamento si trova a 600 metri da Via dell'Indipendenza ea 700 metri da Piazza Maggiore."
        ],
        [
            'city'      => 'Bologna',
            'address'   => 'Via Guglielmo Marconi, 2',
            'name'      => 'La Dotta Apartments',
            'zipcode'   => "40122",
            'description'   => "La Dotta Apartments offre sistemazioni comfort, a 700 metri dal Quadrilatero, a 800 metri da Piazza Maggiore e a meno di 1 km dal Mambo.I luoghi di interesse nelle vicinanze comprendono il Santuario di Santa Maria della Vita, Via dell’Indipendenza e La Macchina del Tempo. L'aeroporto più vicino è quello di Bologna Guglielmo Marconi, a 9 km da La Dotta Apartments."
        ],
        [
            'city'      => 'Bologna',
            'address'   => 'Viale Angelo Masini, 4',
            'name'      => 'Smy Bologna Centrale',
            'zipcode'   => "40126",
            'description'   => "Situato a Bologna, nelle vicinanze dei giardini del Parco della Montagnola, questo hotel di design si trova a soli 150 metri dalla Stazione ferroviaria ed è dotato di camere confortevoli ed eleganti"
        ],
        [
            'city'      => 'Torino',
            'address'   => 'Via Saluzzo, 7',
            'name'      => 'Hotel Urbani',
            'zipcode'   => "10125",
            'description'   => "L'Hotel Urbani è una struttura ricettiva situata a Torino, una città ricca di storia, cultura e bellezze architettoniche. Questo hotel offre una sistemazione confortevole e accogliente per i suoi ospiti."
        ],
        [
            'city'      => 'Torino',
            'address'   => 'Piazza Vittorio Veneto, 13',
            'name'      => 'Piazza Vittorio Suites',
            'zipcode'   => "10124",
            'description'   => "Situato nel centro di Torino, a soli 550 metri dal famoso Museo del Cinema e a 10 minuti a piedi minuti a piedi da Piazza Castello, il Piazza Vittorio Suites offre sistemazioni eleganti con un mix di moderno e classico. "
        ],
        [
            'city'      => 'Torino',
            'address'   => 'Via Santa Teresa, 18',
            'name'      => 'Top Living Apartments - Torino Centro',
            'zipcode'   => "10121",
            'description'   => "Il Top Living Apartments - Torino Centro offre sistemazioni nel centro di Torino, a meno di 1 km dalla stazione della metropolitana Porta Nuova, a 11 minuti a piedi dalla stazione ferroviaria di Porta Nuova e a 1,3 km dalla stazione ferroviaria di Porta Susa."
        ],
        [
            'city'      => 'Torino',
            'address'   => 'Via A. Bonafous, 5',
            'name'      => 'Hotel Alpi Resort',
            'zipcode'   => "10123",
            'description'   => "L'Hotel Alpi Resort, situato a 10 minuti di cammino dal famoso Museo Egizio di Torino e dalla Mole Antonelliana, sarà lieto di accogliervi alla reception, aperta 24 ore su 24."
        ],
        [
            'city'      => 'Napoli',
            'address'   => 'Via Ferdinando Palasciano, 17',
            'name'      => 'Due Cerase a Chiaia',
            'zipcode'   => "80122",
            'description'   => "Situato a Napoli, a meno di 1 km dalla spiaggia di Mappatella, il Due Cerase a Chiaia offre sistemazioni comfortevoli. Situato in un edificio risalente al 1930, questo appartamento si trova a 2,3 km dal Bagno Elena e a 2,4 km dal Bagno Ideal."
        ],
        [
            'city'      => 'Napoli',
            'address'   => 'Discesa Gaiola, 73',
            'name'      => 'Residenza Le Rose Villa d Arte',
            'zipcode'   => "80123",
            'description'   => "Situata nel quartiere di Posillipo, a metà strada fra il Golfo di Napoli e Pozzuoli, la Residenza Le Rose Villa D'Arte offre camere, appartamenti e una collezione d'arte permanente."
        ],
        [
            'city'      => 'Napoli',
            'address'   => 'Via Cesare Rosaroll 15',
            'name'      => 'Le Porte di Napoli Rooms',
            'zipcode'   => "80139",
            'description'   => " La pensione fornisce una sala comune e si trova a meno di 1 km dalla Stazione Ferroviaria di Napoli Centrale, a 1,2 km dalla Chiesa dei Santi Filippo e Giacomo e a 1,8 km dal Museo Archeologico Nazionale di Napoli. Questa struttura non fumatori sorge a 1,9 km dalle Catacombe di San Gaudioso."
        ],
        [
            'city'      => 'Napoli',
            'address'   => 'Via Genova, 63',
            'name'      => 'Atlantic Ostello B&B',
            'zipcode'   => "80143",
            'description'   => "La struttura sorge a 3,6 km dal MUSA, a 3,9 km dal Museo Cappella Sansevero e a circa 3,3 km dalle Catacombe di San Gaudioso. L'ostello dista 4,2 km dal Maschio Angioino e 5,1 km dal Molo Beverello."
        ],
        [
            'city'      => 'Palermo',
            'address'   => 'Via Enrico Toti, 112',
            'name'      => 'Le stanze di Aurora',
            'zipcode'   => "90128",
            'description'   => "Situato a Palermo, a 1,3 km dalla Cattedrale di Palermo, Le Stanze Di Aurora offre ai propri alloggiatori un soggiorno fantastico in una delle città storiche più belle di tutta Italia."
        ],
        [
            'city'      => 'Palermo',
            'address'   => 'Via Divisi, 74',
            'name'      => 'Divisi Suites and Spa',
            'zipcode'   => "90133",
            'description'   => "Dista circa 400 metri dalla Stazione Ferroviaria Centrale di Palermo, 1,1 km dal Teatro Massimo e 1,6 km dal Teatro Politeama."
        ],
        [
            'city'      => 'Palermo',
            'address'   => 'Cortile Giorgio II Greco 1',
            'name'      => 'Cortile Del Greco Apartments',
            'zipcode'   => "90134",
            'description'   => "Il nostro hotel offre un ambiente accogliente e confortevole per il vostro soggiorno. Le nostre camere sono arredate con gusto e pensate per garantire il massimo comfort e relax durante il vostro viaggio. Potrete godere di un'ampia gamma di servizi e comfort durante il vostro soggiorno."
        ],
        [
            'city'      => 'Palermo',
            'address'   => 'Via San Gregorio, 18',
            'name'      => 'Mosina Apartments',
            'zipcode'   => "90134",
            'description'   => "Situato a Palermo, a 600 metri dal duomo cittadino e a meno di 1 km dalla Fontana Pretoria, il Mosina Apartments offre la vista sulla città"
        ],
        [
            'city'      => 'Bari',
            'address'   => 'Via Nicolò Piccinni, 191',
            'name'      => 'Le Stanze di Sofia',
            'zipcode'   => "70122",
            'description'   => "La struttura Le Stanze di Sofia è situata a Bari, a 700 metri dalla cattedrale e a 1 km dal Teatro Margherita. La sistemazione si trova vicino a Piazza del Ferrarese, al Teatro Petruzzelli e a Piazza Mercantile. L’affittacamere dista 400 metri dal centro della città e 2,6 km dalla Spiaggia di Pane e Pomodoro."
        ],
        [
            'city'      => 'Bari',
            'address'   => 'Via Abate Giacinto Gimma, 315',
            'name'      => 'VGM - Apartments Gimma',
            'zipcode'   => "70121",
            'description'   => "Il VGM - Apartments Gimma sorge a Bari, a 2,9 km dalla spiaggia Pane e Pomodoro e a 1,5 km dalla Basilica di San Nicola."
        ],
        [
            'city'      => 'Bari',
            'address'   => 'Strada Nuccia Serra, 29',
            'name'      => 'Palazzo San Rocco',
            'zipcode'   => "70122",
            'description'   => "Il nostro hotel offre un ambiente accogliente e confortevole per il vostro soggiorno. Le nostre camere sono arredate con gusto e pensate per garantire il massimo comfort e relax durante il vostro viaggio. Potrete godere di un'ampia gamma di servizi e comfort durante il vostro soggiorno."
        ],
        [
            'city'      => 'Bari',
            'address'   => 'Via Sagarriga Visconti, 196',
            'name'      => 'La Pietra di San Tommaso - Central Station',
            'zipcode'   => "70122",
            'description'   => "Situato in un edificio storico ricco di eleganza e stile antico, a soli 2 km da Polignano a Mare, il San Tommaso Hotel vanta lussuose camere."
        ],
        [
            'city'      => 'Ravenna',
            'address'   => 'Via Maggiore, 189',
            'name'      => 'Le Camere di Olivia',
            'zipcode'   => "48121",
            'description'   => ""
        ],
        [
            'city'      => 'Ravenna',
            'address'   => 'Via Dismano Vecchio, 38',
            'name'      => 'La Casina dell Artista',
            'zipcode'   => "48121",
            'description'   => ""
        ],
        [
            'city'      => 'Ravenna',
            'address'   => 'Via Cerchio, 74',
            'name'      => 'Atmosfera B B',
            'zipcode'   => "48121",
            'description'   => ""
        ],
        [
            'city'      => 'Ravenna',
            'address'   => 'Via di Roma, 140',
            'name'      => 'Palazzo Galletti Abbiosi',
            'zipcode'   => "48121",
            'description'   => ""
        ],
        [
            'city'      => 'San Teodoro',
            'address'   => 'Località Badualga 1',
            'name'      => 'Hotel San Teodoro',
            'zipcode'   => "07052",
            'description'   => ""
        ],
        [
            'city'      => 'San Teodoro',
            'address'   => 'Via Li Menduli 1',
            'name'      => 'Residence Li Menduli',
            'zipcode'   => "08020",
            'description'   => ""
        ],
        [
            'city'      => 'San Teodoro',
            'address'   => 'Via dei Gerani 1',
            'name'      => 'Gerani Bilocale',
            'zipcode'   => "07052",
            'description'   => ""
        ],
        [
            'city'      => 'San Teodoro',
            'address'   => 'Via di Li Muntigghieddi, snc 1',
            'name'      => 'Li Muntigeddi',
            'zipcode'   => "07052",
            'description'   => ""
        ],
        [
            'city'      => 'Cortina d Ampezzo',
            'address'   => 'Via Val di Sotto, 12',
            'name'      => 'Hotel Corona',
            'zipcode'   => "32043",
            'description'   => ""
        ],
        [
            'city'      => 'Cortina d Ampezzo',
            'address'   => 'Via Stazione, 2',
            'name'      => 'Royal Hotel Cortina',
            'zipcode'   => "32043",
            'description'   => ""
        ],
        [
            'city'      => 'Cortina d Ampezzo',
            'address'   => 'Acquabona di Sotto 1',
            'name'      => 'The Wooden House Cortina',
            'zipcode'   => "32043",
            'description'   => ""
        ],
        [
            'city'      => 'Cortina d Ampezzo',
            'address'   => 'Catarina-Lanz, 99',
            'name'      => 'Les Ciases Chalets Dolomites Apt 3',
            'zipcode'   => "32043",
            'description'   => ""
        ],
        [
            'city'      => 'Perugia',
            'address'   => 'Via Matteo Renato Imbriani, 39',
            'name'      => 'Villa Imbriani',
            'zipcode'   => "06122",
            'description'   => ""
        ],
        [
            'city'      => 'Perugia',
            'address'   => 'Via del Grillo 1',
            'name'      => 'La Corte del Grillo',
            'zipcode'   => "06121",
            'description'   => ""
        ],
        [
            'city'      => 'Perugia',
            'address'   => 'Via del Discobolo, 42',
            'name'      => 'Hotel la Meridiana',
            'zipcode'   => "06132",
            'description'   => ""
        ],
        [
            'city'      => 'Perugia',
            'address'   => 'Via degli Orti 1',
            'name'      => 'BB Uva e Fragola',
            'zipcode'   => "06123",
            'description'   => ""
        ],
    ];